...
//-----------------------------------------------------------------
//------- Macro ---------------------------------------------------
void Bin(cla& a, string jsonfile_module_config){
  // --- ANA CONFIG -----------------------------------------------
...
;
  ...
  // --- HARD CODE -----------------------------------------------
  // OUTPUT
  TString out_file = output_ana_folder+Form("Bin_scan_chi2_lungo_34393");
...
if(ch_rms[1].second[j] == 1000 && int(ch_rms[2].second[j]) == ch){
      ...
      // --- CHI QUADRO --------------------------------------------
      double chi2 = a.fgaus->GetChisquare();
      int ndf = a.fgaus->GetNDF();
      double chi2_ndf = chi2 / ndf;
      cout << "chi quadro ridotto = " << chi2_ndf << endl; 
      ...
      feature_value.push_back({"Bin", a.nbins});
      ...
      feature_value.push_back({"Chi quadro", double(chi2_ndf)});
